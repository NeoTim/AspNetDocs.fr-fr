...
Notez que le modèle de vue contient une `@ModelType MvcMovie.Models.Movie` instruction en haut du fichier, ce qui indique que la vue s’attend à ce que le modèle du modèle de vue soit de type `Movie` .
...
Si les valeurs publiées ne sont pas valides, elles sont réaffichées dans le formulaire. Les `Html.ValidationMessageFor` applications auxiliaires dans le modèle de vue *Edit. vbhtml* prennent en charge l’affichage des messages d’erreur appropriés.
...
S’il ne s’agit pas de String. IsNullOrEmpty (searchString),   
 movies = films. Where (fonction (s) s. title. Contains (searchString))   
 Terminer si
...
Lorsque vous cliquez sur le bouton **Ajouter** , le modèle de vue *Views\Movies\SearchIndex.vbhtml* est créé. Dans la mesure où vous avez sélectionné la **liste** dans la liste **modèle de structure** , Visual Web Developer a généré automatiquement un contenu par défaut dans la vue. La génération de modèles automatique a créé un formulaire HTML. Il a examiné la `Movie` classe et créé du code pour afficher les `<label>` éléments de chaque propriété de la classe. La liste ci-dessous montre la vue Create qui a été générée :
...
Ouvrez le fichier *Views\Movies\SearchIndex.vbhtml* et, juste après `@Html.ActionLink("Create New", "Create")` , ajoutez ce qui suit :
...
Il n’y a aucune `HttpPost` surcharge de la `SearchIndex` méthode. Vous n’en avez pas besoin, car la méthode ne modifie pas l’état de l’application, il suffit de filtrer les données. Si vous avez ajouté la `HttpPost` `SearchIndex` méthode suivante, le demandeur d’action correspondrait à la `HttpPost` `SearchIndex` méthode et la `HttpPost` `SearchIndex` méthode s’exécuterait comme indiqué dans l’image ci-dessous.
...
Ajoutez une `Html.DropDownList` application d’assistance au fichier *Views\Movies\SearchIndex.vbhtml* , juste avant l' `TextBox` application auxiliaire. Le balisage terminé est illustré ci-dessous :
