---
uid: web-pages/index
title: ASP.NET Pages Web (fr) Microsoft Docs
author: rick-anderson
description: Téléchargez WebMatrix & apprendre à construire rapidement des pages Web d’une manière légère pour combiner le code serveur avec HTML.
ms.author: riande
ms.date: 05/18/2012
ms.assetid: 5418ba96-ff49-4a43-9cbb-09dea4949d23
msc.legacyurl: /web-pages
msc.type: categoryindex
ms.openlocfilehash: d767325a376e5daa0ea4236f67c88e6b97ae71a7
ms.sourcegitcommit: 022f79dbc1350e0c6ffaa1e7e7c6e850cdabf9af
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 04/17/2020
ms.locfileid: "81543520"
---
# <a name="aspnet-web-pages"></a>Pages web ASP.NET

> [!NOTE] 
> WebMatrix n’est plus recommandé comme environnement de développement intégré pour ASP.NET pages Web. Utilisez [Visual Studio](xref:web-pages/overview/getting-started/program-asp-net-web-pages-in-visual-studio) ou Visual Studio [Code](https://code.visualstudio.com/).

- [Assistance](overview/index.md)

    - [Prise en main](overview/getting-started/index.md)

        - [Programmer ASP.NET Web Pages dans Visual Studio](overview/getting-started/program-asp-net-web-pages-in-visual-studio.md)
        - [Introduction à la syntaxe Razor de la programmation web ASP.NET](overview/getting-started/introducing-razor-syntax-c.md)
        - [ASP.NET Web Pages - Visual Basic](overview/getting-started/introducing-razor-syntax-vb.md)
        - [Pages web ASP.NET - Questions fréquentes (FAQ) (Razor)](overview/getting-started/aspnet-web-pages-razor-faq.md)
        - [Envoi d’e-mails depuis votre site](overview/getting-started/11-adding-email-to-your-web-site.md)
        - [Ajout de fonctionnalités de réseaux sociaux à votre site web](overview/getting-started/13-adding-social-networking-to-your-web-site.md)
        - [Introduction à ASP.NET Web Pages](overview/getting-started/introducing-aspnet-web-pages-2/index.md)

            - [Prise en main](overview/getting-started/introducing-aspnet-web-pages-2/getting-started.md)
            - [Principes de base de la programmation](overview/getting-started/introducing-aspnet-web-pages-2/intro-to-web-pages-programming.md)
            - [Affichage des données](overview/getting-started/introducing-aspnet-web-pages-2/displaying-data.md)
            - [Principes de base des formulaires HTML](overview/getting-started/introducing-aspnet-web-pages-2/form-basics.md)
            - [Saisie de données de la base de données avec des formulaires](overview/getting-started/introducing-aspnet-web-pages-2/entering-data.md)
            - [Mise à jour de données de la base de données](overview/getting-started/introducing-aspnet-web-pages-2/updating-data.md)
            - [Suppression de données de la base de données](overview/getting-started/introducing-aspnet-web-pages-2/deleting-data.md)
            - [Création d’une disposition cohérente](overview/getting-started/introducing-aspnet-web-pages-2/layouts.md)
            - [Publication d’un site avec WebMatrix](overview/getting-started/introducing-aspnet-web-pages-2/publishing.md)
    - [Routage](overview/routing/index.md)

        - [Création d’URL lisibles dans les sites ASP.NET Web Pages](overview/routing/creating-readable-urls-in-aspnet-web-pages-sites.md)
    - [Interface utilisateur, dispositions et thèmes](overview/ui-layouts-and-themes/index.md)

        - [Utilisation des formulaires](overview/ui-layouts-and-themes/4-working-with-forms.md)
        - [Validation des entrées utilisateur dans les sites ASP.NET Web Pages](overview/ui-layouts-and-themes/validating-user-input-in-aspnet-web-pages-sites.md)
        - [Création d’une apparence cohérente](overview/ui-layouts-and-themes/3-creating-a-consistent-look.md)
        - [Personnalisation du comportement à l’échelle du site](overview/ui-layouts-and-themes/18-customizing-site-wide-behavior.md)
        - [Travailler avec Images](overview/ui-layouts-and-themes/9-working-with-images.md)
        - [Utilisation de la vidéo](overview/ui-layouts-and-themes/10-working-with-video.md)
        - [Création et utilisation d’un helper dans un site ASP.NET Web Pages](overview/ui-layouts-and-themes/creating-and-using-a-helper-in-an-aspnet-web-pages-site.md)
        - [Installation d’un helper](overview/ui-layouts-and-themes/installing-helpers.md)
        - [Helper Twitter avec ASP.NET Web Pages](overview/ui-layouts-and-themes/twitter-helper.md)
        - [Affichage des mappages dans un site ASP.NET Web Pages](overview/ui-layouts-and-themes/displaying-maps-in-an-aspnet-web-pages-site.md)
    - [Mobile](overview/mobile/index.md)

        - [Rendu des sites ASP.NET Web Pages pour les appareils mobiles](overview/mobile/rendering-aspnet-web-pages-sites-for-mobile-devices.md)
    - [Travailler avec les données](overview/data/index.md)

        - [Travailler avec les données](overview/data/5-working-with-data.md)
        - [Affichage de données dans un graphique](overview/data/7-displaying-data-in-a-chart.md)
        - [Utilisation de fichiers](overview/data/working-with-files.md)
    - [Tests et débogage](overview/testing-and-debugging/index.md)

        - [Introduction au débogage](overview/testing-and-debugging/introduction-to-debugging.md)
        - [ASP.NET Web Pages (Razor) - Guide de résolution des problèmes](overview/testing-and-debugging/aspnet-web-pages-razor-troubleshooting-guide.md)
    - [Sécurité](overview/security/index.md)

        - [Ajout de la sécurité et de l’appartenance](overview/security/16-adding-security-and-membership.md)
        - [Activation de la connexion à partir de sites externes dans un site ASP.NET Web Pages](overview/security/enabling-login-from-external-sites-in-an-aspnet-web-pages-site.md)
        - [Utilisation d’un CAPTCHA pour empêcher les programmes automatisés (robots) d’utiliser votre site web ASP.NET](overview/security/using-a-catpcha-to-prevent-automated-programs-bots-from-using-your-aspnet-web-site.md)
    - [Performances](overview/performance-and-traffic/index.md)

        - [Mise en cache pour améliorer les performances de votre site web](overview/performance-and-traffic/15-caching-to-improve-the-performance-of-your-website.md)
        - [Analyse du trafic](overview/performance-and-traffic/14-analyzing-traffic.md)
        - [Bundles et minimisation des ressources dans un site ASP.NET Web Pages (Razor)](overview/performance-and-traffic/bundling-and-minifying-assets-in-an-aspnet-web-pages-razor-site.md)
    - [Référence des API](overview/api-reference/index.md)

        - [Aide-mémoire de l’API ASP.NET](overview/api-reference/asp-net-web-pages-api-reference.md)
    - [Publications](overview/releases/index.md)

        - [Nouveautés d’ASP.NET Web Pages 3.2](overview/releases/whats-new-in-aspnet-web-pages-32.md)
        - [Nouveautés d’ASP.NET Web Pages 3.1](overview/releases/whats-new-aspnet-web-pages-31.md)
        - [Fonctionnalités principales d’ASP.NET Web Pages 2](overview/releases/top-features-in-web-pages-2.md)
        - [Fichier Lisez-moi d’ASP.NET Web Pages 2 Developer Preview](overview/releases/aspnet-web-pages-2-developer-preview-readme.md)
        - [Exécution côte à côte de différentes versions d’ASP.NET Web Pages](overview/releases/running-v1-and-v2-sites-side-by-side.md)
- [Vidéos](videos/index.md)

    - [Introduction](videos/introduction/index.md)

        - [Introduction à ASP.NET Web Pages (5 minutes)](videos/introduction/5-minute-introduction-to-aspnet-web-pages.md)
        - [Installer et utiliser l’outil Microsoft WebMatrix](videos/introduction/install-and-use-the-microsoft-webmatrix-tool.md)
        - [Créer un site web avec WebMatrix](videos/introduction/create-a-website-using-webmatrix.md)
        - [Modifier le style visuel d’une page web](videos/introduction/change-the-visual-style-of-a-web-page.md)
        - [Utiliser les fonctionnalités de disposition dans WebMatrix](videos/introduction/use-the-layout-features-in-webmatrix.md)
        - [Créer une page web dynamique pilotée par les données](videos/introduction/create-a-data-driven-dynamic-web-page.md)
        - [Créer une interface web dans WebMatrix](videos/introduction/create-a-web-interface-in-webmatrix.md)
        - [Créer une page de modification des données dans WebMatrix](videos/introduction/create-an-edit-data-page-in-webmatrix.md)
        - [Créer une page de suppression des données dans WebMatrix](videos/introduction/create-a-delete-data-page-in-webmatrix.md)
        - [Publier un site web sur Internet avec WebMatrix](videos/introduction/publish-a-website-to-the-internet-using-webmatrix.md)
    - [ASP.NET Razor Pages](videos/aspnet-razor-pages/index.md)

        - [Bien démarrer avec WebMatrix et ASP.NET Web Pages](videos/aspnet-razor-pages/getting-started-with-webmatrix-and-aspnet-web-pages.md)
        - [Introduction à la syntaxe Razor de la programmation web ASP.NET](videos/aspnet-razor-pages/introduction-to-aspnet-web-programming-using-the-razor-syntax.md)
        - [Création d’une apparence cohérente (Partie 1)](videos/aspnet-razor-pages/creating-a-consistent-look-part-1.md)
        - [Création d’une apparence cohérente (Partie 2)](videos/aspnet-razor-pages/creating-a-consistent-look-part-2.md)
        - [Utilisation des formulaires (Partie 1)](videos/aspnet-razor-pages/working-with-forms-part-1.md)
        - [Utilisation des formulaires (Partie 2)](videos/aspnet-razor-pages/working-with-forms-part-2.md)
        - [Utilisation des données (Partie 1)](videos/aspnet-razor-pages/working-with-data-part-1.md)
        - [Utilisation des données (Partie 2)](videos/aspnet-razor-pages/working-with-data-part-2.md)
        - [Affichage de données dans une grille](videos/aspnet-razor-pages/displaying-data-in-a-grid.md)
        - [Affichage de données dans un graphique (Partie 1)](videos/aspnet-razor-pages/displaying-data-in-a-chart-part-1.md)
        - [Affichage de données dans un graphique (Partie 2)](videos/aspnet-razor-pages/displaying-data-in-a-chart-part-2.md)
        - [Utilisation de fichiers](videos/aspnet-razor-pages/working-with-files.md)
        - [Travailler avec Images](videos/aspnet-razor-pages/working-with-images.md)
        - [Utilisation de la vidéo](videos/aspnet-razor-pages/working-with-video.md)
        - [Ajout de fonctionnalités d’e-mail à votre site web](videos/aspnet-razor-pages/adding-email-to-your-web-site.md)
        - [Ajout de fonctionnalités de recherche à votre site web](videos/aspnet-razor-pages/adding-search-to-your-web-site.md)
        - [Ajout de fonctionnalités de réseaux sociaux à votre site web](videos/aspnet-razor-pages/adding-social-networking-to-your-website.md)
- [Vidéo de formation à WebMatrix et à Web de Pluralsight](pluralsight.md)
- [Fichier Lisez-moi de WebMatrix](readme/index.md)

    - [Fichier Lisez-moi de WebMatrix](readme/overview.md)
    - [Fichier Lisez-moi de WebMatrix et ASP.NET Web Pages (Razor) Bêta 3](readme/beta3.md)
- [Guide de contenu de WebMatrix](content-guide.md)
